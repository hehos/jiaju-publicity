  ...
  var encryptData = rsaEncryptNameAndPhone({phoneObj:thisForm.find('input[name=phone]'),chenhuObj:thisForm.find('input[name=chenhu]')});
    ...
    jq.ajax({
        ...
        success: function(result) {
            ...
            yuyue_apply_agin = 0;
            if (res.status == 1) {
                showPopWin('http://www.to8to.com/ask/popdialog.php?info=1', 369, 145, null, false);
                return false;
                jq("#izxlc_cmdl_" + id + "_" + sid).html('<a target="_blank" href="http://bbs.to8to.com/forum-1582-1.html"><img src="http://img.to8to.com/front_end/pic/index_v3_tjym.jpg" /></a>')
            }
            else if(res.status == 5)
            {
                showPopWin("http://www.to8to.com/zb/frame_global.php?status=" + res.status + "&msg=" + res.cityname, 456, 254, null, true);
                return false;
            }
            else
            {
...
function freeZhaoBiao(n){
    ...
    jq.ajax({
        ...
        success: function(result) {
			...
			// try{
			// 	jq('#tj').html('<iframe src="http://www.to8to.com/api/test.html"></iframe>');
			// }catch(e){}
            // alert(res.req);
...
function sjZhaoBiao(){
    ...
    jq.ajax({
        type: "POST",
        url: "http://sjs.to8to.com/zb/zbAjax.php",
    ...
    jq.prompt(txt,{
        ...
        callback: function(v,m,f){
            if(v == 1){
                ...
                window.location.href='http://www.to8to.com/reg/';   
            }
        }
    });
}
